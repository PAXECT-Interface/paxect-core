...
# PAXECT Core — Production Plugin (ready-to-use)
#
# Core features:
# - Container format with fixed header/footer (VERSION=42, backward compatible)
# - Multi-channel support: 1–8 channels (default=1)
# - Auto mode: automatically selects channel count, frame size, and threads based on RAM/CPU
# - Streaming encode/decode using Zstandard (adjustable compression level)
# - Frames contain channel ID + CRC32 per frame
# - Footer includes total uncompressed length + SHA-256 of the original data
# - Mapping metadata: "virtual" (0) or "u16" (1)
# - Supports stdin/stdout (binary streams)
# - Exit codes:
#       0 = OK
...
# Flags
FLAG_VERIFY = 1 << 0       # When set: footer.sha256 must be verified during decode
...
def parse_args(argv=None):
    p = argparse.ArgumentParser(prog="paxect-core", description="PAXECT Core Production Plugin (multi-channel, auto-mode, cross-platform)")
    ...
    pe.add_argument("--level", type=int, default=5, help="zstd level (1-19)")
    ...
    pe.add_argument("--mapping", choices=["virtual", "u16"], default="virtual")
